...
import { FinanceLoanAppliedPayload } from '../types/index.js';
...
const TOPIC = 'finance.loan.applied';

export async function financeLoanAppliedHandler(raw: unknown): Promise<void> {
  const result = FinanceLoanAppliedPayload.safeParse(raw);
  if (!result.success) {
    logger.warn({ errors: result.error.flatten(), context: 'financeLoanAppliedHandler' }, 'invalid payload');
    return;
  }
  ...
  if (token) {
    ...
    logger.warn({ farmerId, context: 'financeLoanAppliedHandler' }, 'no FCM token — skipping push');
  }
  ...
  logger.info({ farmerId, loanId }, 'financeLoanAppliedHandler done');
}
