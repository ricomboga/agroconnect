...
import { useNgoFarmerCreation, type PlotDraft } from '../_context/NgoFarmerCreationContext'
...
export default function NgoFarmerStep3Page() {
  ...
  return (
    <div>
      <FormSection title="Farm Basics">
        <FieldGroup cols={2}>
          ...
          <Field label="Farming System">
            <Select value={state.farmingSystem} onChange={(e) => update({ farmingSystem: e.target.value })}>
              <option value="">Select…</option>
      ...
      <FormSection title="GPS Validation">
        ...
        {state.gpsValidation && !state.gpsValidation.valid && (
          <AlertBox variant="red">
            ...
            {state.gpsValidation.distanceKm} km away). Please re-enter or confirm the county.
          </AlertBox>
        )}
      ...
      <FormSection title="Plots & Crops">
        <AlertBox variant="green">
          Saving this farm auto-generates an AI activity schedule for each plot&apos;s crop from its planting
          date.
        </AlertBox>
        {state.plots.map((plot, idx) => (
          <div key={plot.id} className="rounded-base border border-border bg-surface p-3">
            ...
            <FieldGroup cols={2}>
              ...
              <Field label="Season">
                <Select value={plot.season} onChange={(e) => updatePlot(plot.id, { season: e.target.value })}>
                  <option value="">Select…</option>
              ...
              <Field label="Seed Source">
                <Select value={plot.seedSource} onChange={(e) => updatePlot(plot.id, { seedSource: e.target.value })}>
                  <option value="">Select…</option>
      ...
      <div className="flex gap-2">
        <button
          type="button"
          onClick={() => router.push('/lender/farmers/new/2')}
        ...
        <button
          type="button"
          onClick={() => {
            if (validate()) router.push('/lender/farmers/new/4')
          }}
