...
export default function NgoFarmerStep2Page() {
  ...
  function handleNext() {
    ...
    router.push('/lender/farmers/new/3')
  }
    ...
    <div>
      ...
      {showCrops && (
        <FormSection title="Primary Crops (select all)">
          <ChipSelect options={CROP_OPTIONS} multiple value={state.crops} onChange={(v) => update({ crops: v })} />
        </FormSection>
      )}
                  ...
                  .map((t) => (
                    <Field key={t} label={LIVESTOCK_COUNT_LABELS[t]}>
                      <TextInput
                        ...
                        onChange={(e) =>
                          update({ livestockCounts: { ...state.livestockCounts, [t]: e.target.value } })
                        }
                      />
                    </Field>
                  ))}
      ...
      <FormSection title="Experience Level">
        <FieldGroup cols={2}>
          <Field label="Years farming">
            <Select value={state.experienceYears} onChange={(e) => update({ experienceYears: e.target.value })}>
          ...
          <Field label="Primary income source">
            <Select value={state.incomeSource} onChange={(e) => update({ incomeSource: e.target.value })}>
              <option value="">Select…</option>
      ...
      <div className="flex gap-2">
        <button
          type="button"
          onClick={() => router.push('/lender/farmers/new')}
