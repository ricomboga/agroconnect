...
| GET | /finance/farmers/lender-map?farmerIds=... | JWT (admin) | For a set of farmer IDs, returns which ones already have a lender/NGO/Group assignment and its name — `{ [farmerId]: { lenderId, lenderName } }`, omitting unassigned farmers. Powers the "NGO/Group" column on the admin farmer list. |
